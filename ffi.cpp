...
// nanobind headers FIRST is important
...
Interval scalarToInterval(const double &x, double half_range = 0.0)
{
  double epsilon = std::max(std::abs(x), 1.0) * 1e-15;
  double lower = x - half_range - epsilon;
  double upper = x + half_range + epsilon;
  return Interval(lower, upper);
}

Interval scalarToInterval(const double &x)
{
  return scalarToInterval(x, 0.0);
}

NB_MODULE(ffi_module, m)
{
  // more precise interval definition
  nb::class_<Interval>(m, "Interval")
      .def("__init__", [](Interval *self, double lower, double upper)
           {
          // Ensure lower <= upper
          if (lower > upper) std::swap(lower, upper);
          // Add epsilon to guarantee containment
          double epsilon = std::max(std::abs(lower), std::abs(upper)) * 1e-15 + 1e-300;
          new (self) Interval(lower - epsilon, upper + epsilon); })
  ...
  m.def("scalarToInterval", (Interval (*)(const double &, double))&scalarToInterval);
  m.def("scalarToInterval", (Interval (*)(const double &))&scalarToInterval);
}